...
ROPNEW;
Edge *head[510], *cur[510];

basic_string < int > ans[310];
...
int main(){
    ...
    M = read(), N = read();
    S = N + M + 1, T = N + M + 2;
    for(int i = 1; i <= M; ++i)sum += (R[i] = read());
    for(int i = 1; i <= N; ++i)C[i] = read();
    for(int i = 1; i <= M; ++i)AddEdge(S, i, R[i]);
    for(int i = 1; i <= N; ++i)AddEdge(M + i, T, C[i]);
    for(int i = 1; i <= M; ++i)for(int j = 1; j <= N; ++j)AddEdge(i, M + j);
    auto bfs = [](void)->bool{
        memset(dep, 0, sizeof dep); dep[T] = 1;
        copy(head + 1, head + N + M + 2 + 1, cur + 1);
    ...
    for(int p = 1; p <= M; ++p)
        for(auto i = head[p]; i; i = i->nxt)
            if(i->val == 0 && M + 1 <= SON && SON <= M + N)ans[p] += SON - M;
    for(int i = 1; i <= M; ++i){
