...
ROPNEW;
Edge *head[2100], *cur[2100];

basic_string < int > ans[1100];
...
int main(){
    ...
    K = read(), N = read();
    S = N + K + 1, T = N + K + 2;
    for(int i = 1; i <= K; ++i)sum += (R[i] = read());
    for(int i = 1; i <= K; ++i)AddEdge(N + i, T, R[i]);
    for(int i = 1; i <= N; ++i){
        AddEdge(S, i, 1);
        int cnt = read();
        while(cnt--){
    ...
    auto bfs = [](void)->bool{
        ...
        copy(head + 1, head + N + K + 2 + 1, cur + 1);
    ...
    for(int p = 1; p <= N; ++p)
        for(auto i = head[p]; i; i = i->nxt)
            if(i->val == 0 && N + 1 <= SON && SON <= N + K)ans[SON - N] += p;
    for(int i = 1; i <= K; ++i){
        printf("%d: ", i);
