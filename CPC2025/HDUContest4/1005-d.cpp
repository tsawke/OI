            ...
            for(int p = 1; p <= N; ++p){
                ...
                if(opt == 'L' || opt == 'R'){
                    if(isRoot[p])dp[m][p] = dp[m + 1][p];
                        ...
                        dp[m][p] = (double)sum / (double)deg_son[fa[p]];//* inv[deg_son[fa[p]]]// % MOD;
                    }
                }else{ // opt == 'D'
                    if(isLeaf[p])dp[m][p] = dp[m + 1][p];
                        ...
                        for(auto i = head[p]; i; i = i->nxt)
                            sum += dp[m + 1][i->to];//, sum %= MOD;
                        dp[m][p] = (double)sum / (double)deg_son[p];//sum * inv[deg_son[p]] % MOD;
                    }
                }
            }
        }
        for(int i = 0; i <= M; ++i)for(int j = 1; j <= N; ++j)printf("dp[%d][%d] = %.2lf\n", i, j, dp[i][j]);
        // printf("%lld\n", dp[0][root]);
        cout << dp[0][root] << endl;
    }
