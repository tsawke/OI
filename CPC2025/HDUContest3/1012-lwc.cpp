...
signed main()
{
    ...
    while (T--)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            int t = ((i - 1) * a[i].val % MOD * a[i].x % MOD + suma * a[i].x % MOD) % MOD;
            //		cout<<" mid: "<<i<<" t="<<t<<endl;
            t = ((((t - a[i].val * sumx % MOD) % MOD + MOD) % MOD - sumax) % MOD + MOD) % MOD;
            //		cout<<" mid: "<<i<<" t="<<t<<endl;
            ans = (ans + t) % MOD;
            suma = (suma + a[i].val) % MOD;
            sumx = (sumx + a[i].x) % MOD;
            sumax = (sumax + a[i].x * a[i].val % MOD) % MOD;
        ...
        for (int i = 1; i <= n; i++)
        { // y[i]-y[j]>x[i]-x[j]
            int t = query7(a[i].yx - 1) * a[i].val % MOD * (((a[i].y - a[i].x) % MOD + MOD) % MOD) % MOD;
            t = (((((t - query1(a[i].yx - 1)) % MOD - a[i].val) % MOD + MOD) % MOD * query2(a[i].yx - 1) % MOD + (a[i].y - a[i].x) % MOD * query3(a[i].yx - 1) % MOD + MOD) % MOD) % MOD;
            ans = (ans + t) % MOD;
            add1(a[i].yx, (a[i].val * (a[i].y - a[i].x) % MOD + MOD) % MOD);
            add2(a[i].yx, ((a[i].y - a[i].x) % MOD + MOD) % MOD);
            add3(a[i].yx, a[i].val);
            add7(a[i].yx, 1);
        }
        ...
        for (int i = 1; i <= n; i++)
        { // x[i]+y[i]<x[j]+y[j]
            int t = (-query8(a[i].xy - 1) * a[i].val % MOD * (a[i].x + a[i].y) % MOD + MOD) % MOD;
            t = ((t + query4(a[i].xy - 1) % MOD - query5(a[i].xy - 1) * (a[i].x + a[i].y) % MOD + a[i].val * query6(a[i].xy - 1) % MOD) % MOD + MOD) % MOD;
            t = ((-t + MOD) % MOD + MOD) % MOD;
            ans = (ans + t) % MOD;

            add4(a[i].xy, a[i].val * (a[i].x + a[i].y) % MOD);
            add5(a[i].xy, a[i].val);
            add6(a[i].xy, (a[i].x + a[i].y) % MOD);
            add8(a[i].xy, 1);
        }
