...
int main(){
	char c[110000];
	for(int tmp = 10000; tmp <= 1000000; ++tmp){
		NNN = rndd(1, 200000);
		for(int i = 2; i <= NNN; ++i)a[i] = rndd(1, i - 1);
		QQQ = rndd(1, 400000);
		for(int i = 1; i <= QQQ; ++i){
			x[i] = rndd(1, NNN), y[i] = rndd(1, NNN);
		}
		int Time[10];
		bool isTLE(false);
		Time[1] = A::main(NNN, a, QQQ, x, y); if(Time[1] > 10000000)isTLE = true;
		Time[2] = B::main(NNN, a, QQQ, x, y); if(Time[2] > 10000000)isTLE = true;
		Time[3] = C::main(NNN, a, QQQ, x, y); if(Time[3] > 10000000)isTLE = true;
		Time[4] = D::main(NNN, a, QQQ, x, y); if(Time[4] > 10000000)isTLE = true;
		Time[5] = E::main(NNN, a, QQQ, x, y); if(Time[5] > 10000000)isTLE = true;
		Time[6] = F::main(NNN, a, QQQ, x, y); //if(Time[6] > 10000000)isTLE = true;
		Time[7] = G::main(NNN, a, QQQ, x, y); if(Time[7] > 10000000 && Time[1] < 10000)isTLE = true;
		if(isTLE){
			bool isTalent(false);
			for(int i = 1; i <= 7; ++i)if(Time[i] != -1 && Time[i] < 3000000)isTalent = true;
			if(!isTalent){
				isTLE = false;
				continue;
			}

			freopen("aa-index.txt", "a", stdout);
			fflush(stdout);

			printf("Time %d: ", tmp);
			for(int i = 1; i <= 7; ++i)printf("%d ", Time[i] > 10000000 ? -1 : Time[i]);
			printf("\n");
			// printf("\nValues:\n");

			sprintf(c, "ans%d.txt", tmp);
			freopen(c, "w", stdout);

            printf("%d\n", NNN);
            for(int i = 2; i <= NNN; ++i)printf("%d\n", a[i]);
            printf("%d\n", QQQ);
            for(int i = 1; i <= QQQ; ++i)printf("%d %d\n", x[i], y[i]);




			// int rKey = rndd(1, 114514);

			// printf("N%d=%d;\n", rKey, NNN);
			// printf("a%d[%d] = {0, 0, ", rKey, NNN + 10);
			// for(int i = 2; i <= NNN; ++i){
			// 	printf("%d%c", a[i], i == NNN ? '}' : ',');
			// 	if(i % 100 == 0)printf("\n");
			// }
			// printf(";\n");
			// printf("Q%d=%d;\n", rKey, QQQ);
			// printf("x%d[%d] = {0, ", rKey, QQQ + 10);
			// for(int i = 1; i <= QQQ; ++i){
			// 	printf("%d%c", x[i], i == QQQ ? '}' : ',');
			// 	if(i % 100 == 0)printf("\n");
			// }
			// printf(";\n");
			// printf("Q%d=%d\n", rKey, QQQ);
			// printf("y%d[%d] = {0, ", rKey, QQQ + 10);
			// for(int i = 1; i <= QQQ; ++i){
			// 	printf("%d%c", y[i], i == QQQ ? '}' : ',');
			// 	if(i % 100 == 0)printf("\n");
			// }
			// printf(";\n");
		}
