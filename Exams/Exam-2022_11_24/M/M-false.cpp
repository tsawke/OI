...
int main(){
    mn[0] = (ll)114514 * 114514;
    N = read();
    for(int i = 1; i <= N; ++i)data += a[i] = read();
    sort(data.begin(), data.end());
    // for(int i = 1; i <= N; ++i)base[i] = distance(data.begin(), lower_bound(data.begin(), data.end(), a[i]) + 1), base[i] += cnt[base[i]]++;
    for(int i = 1; i <= N; ++i)dp[i] = abs(a[1] - data.at(i - 1)), mn[i] = min(mn[i - 1], dp[i]);
    for(int i = 1; i <= N; ++i)printf("dp[%d] : %lld\n", i, dp[i]);
    for(int i = 2; i <= N; ++i){
        for(int j = 1; j <= N; ++j)
            dp[j] = mn[j - 1] + (ll)abs(a[i] - data.at(j - 1));
        for(int j = 1; j <= N; ++j)
